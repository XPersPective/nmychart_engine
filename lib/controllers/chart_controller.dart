...
import '../models/chart_field.dart';
...
class ChartController extends ChangeNotifier {
  ...
  void _calculateViewport() {
    ...
    final timeField = _data.timeField;
    if (timeField == null) {
      _viewport = Rect.zero;
      return;
    }
    ...
    for (final point in _data.data) {
      if (point.length > timeField.index) {
      ...
      for (final field in _data.fields) {
        if (field.axis == 'y' && field.type == ChartFieldType.number) {
          if (point.length > field.index) {
  ...
  Range getVisibleRange() {
    ...
    for (int i = 0; i < _data.data.length; i++) {
      final time = (_data.data[i][timeField.index] as num).toDouble();
      if (time >= leftWorld) {
